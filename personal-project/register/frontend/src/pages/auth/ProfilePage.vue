...
// 경로가 변하지 않으므로 새로고침을 해야 Avatar가 바뀐다.
// 따라서 avatar 뒤에 '/랜덤한_숫자'를 붙이면 경로가 변하므로 Vue가 다시 불러온다.
const avatarPath = computed(
  () => `/api/member/${auth.username}/avatar/${auth.avatarRand}`
);
...
const onSubmit = async () => {
  ...
  try {
    ...
    auth.updateAvatarRand(); // avatarRand 갱신
    alert('정보를 수정하였습니다.');
  } catch (e) {
