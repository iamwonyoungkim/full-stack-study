...
public class BoardController {
    ...
    @PostMapping("")
    public ResponseEntity<BoardDTO> create(BoardDTO board) { // Multipart 인코딩이므로 @RequestBody 붙이지 않음
        return ResponseEntity.ok(service.create(board));
    }

    @PutMapping("/{no}")
    public ResponseEntity<BoardDTO> update(@PathVariable Long no, BoardDTO board) { // Multipart 인코딩이므로 @RequestBody 붙이지 않음
